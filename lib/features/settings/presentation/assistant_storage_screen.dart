...
class _AssistantStorageScreenState extends State<AssistantStorageScreen> {
  ...
  Future<void> _load() async {
    final allSup = await FirebaseService.getAssistantSupabaseAccounts();
    final allCloud = await FirebaseService.getAssistantCloudinaryAccounts();
    if (_isScoped) {
      final scopedSup = allSup.where((a) => a['assistantUid'] == widget.assistantUid).toList();
      scopedSup.sort((a, b) {
      ...
      final scopedCloud = allCloud.where((a) => a['assistantUid'] == widget.assistantUid).toList();
      scopedCloud.sort((a, b) {
        final aTime = DateTime.tryParse(a['createdAt'] as String? ?? '') ?? DateTime(1970);
        final bTime = DateTime.tryParse(b['createdAt'] as String? ?? '') ?? DateTime(1970);
        return bTime.compareTo(aTime);
      });
      if (mounted) setState(() {
        _supabaseAccounts = scopedSup;
        _cloudinaryAccounts = scopedCloud;
        _loading = false;
      });
    } else {
      ...
      if (mounted) setState(() {
        ...
        _supabaseAccounts = allSup;
        _cloudinaryAccounts = allCloud;
        _loading = false;
      });
    }
  }
    ...
    return ListView(
      ...
      children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Tap an assistant to manage their storage accounts', style: TextStyle(color: hintColor, fontSize: 12)),
                  ],
                ),
              ),
            ]),
        ...
        if (_assistants.isEmpty)
          _emptyState('No assistants found', 'Create assistants from the admin panel', Icons.person_off_rounded, isDark, hintColor, Colors.orange)
        else
          ...List.generate(_assistants.length, (i) {
            ...
            final supAccounts = _supabaseAccounts.where((a) => a['assistantUid'] == uid).toList();
            final cloudAccounts = _cloudinaryAccounts.where((a) => a['assistantUid'] == uid).toList();
            final activeSup = supAccounts.where((a) => a['isActive'] == true).toList();
            final activeCloud = cloudAccounts.where((a) => a['isActive'] == true).toList();
            final activeUrl = activeSup.isNotEmpty ? (activeSup.first['projectUrl'] as String? ?? '') : '';
            final activeCloudName = activeCloud.isNotEmpty ? (activeCloud.first['cloudName'] as String? ?? '') : '';
            final totalAccounts = supAccounts.length + cloudAccounts.length;
            final statusText = totalAccounts == 0
                ? 'No storage assigned'
                : '${supAccounts.length} Supabase, ${cloudAccounts.length} Cloudinary';
            return InkWell(
              onTap: () async {
                ...
                child: Row(children: [
                  ...
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      Text(statusText, style: TextStyle(color: (activeUrl.isNotEmpty || activeCloudName.isNotEmpty) ? Colors.teal : hintColor, fontSize: 12), overflow: TextOverflow.ellipsis),
                    ]),
                  ),
            ...
            child: Column(
              ...
              children: [
                ...
                if (_supabaseAccounts.isEmpty)
                  _emptyState('No assistant Supabase accounts', 'Assign Supabase accounts to assistants', Icons.person_add_rounded, isDark, hintColor, Colors.teal)
                else
            ...
            child: Column(
              ...
              children: [
                ...
                if (_cloudinaryAccounts.isEmpty)
                  _emptyState('No assistant Cloudinary accounts', 'Assign Cloudinary accounts to assistants', Icons.person_add_rounded, isDark, hintColor, Colors.deepPurple)
                else
          ...
          child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            if (scopedUid == null)
              ...
              Container(
                ...
                child: Row(children: [
                  const Icon(Icons.person_rounded, color: Colors.teal, size: 18),
                  const SizedBox(width: 8),
          ...
          ElevatedButton(
            onPressed: isLoading ? null : () async {
              ...
              if (verifyResult['valid'] == true) {
                ...
                // Auto-poll: check initial storage usage
                try {
